...
import { createQualityAssessmentSchema } from '@/modules/analysis-records/schema'
import { analysisRecordService } from '@/modules/analysis-records/service'

export const runtime = 'nodejs'

/** Stored fruit-quality assessments the caller may see, newest first. */
export async function GET(request: NextRequest) {
  try {
    ...
    const { items, total } = await analysisRecordService.listQuality(
    ...
    logger.error({ err: error }, 'failed to list fruit quality assessments')
    return serverError()
  }
}

/** Store the result of an uploaded image/video analysis (live results are stored by the camera route). */
export async function POST(request: NextRequest) {
  try {
    const session = await auth()
    if (!session) return unauthorized()
    checkPermission(session, 'camera', 'read')
    const parsed = createQualityAssessmentSchema.safeParse(await request.json().catch(() => null))
    if (!parsed.success) return validationError(parsed.error)
    const target = await resolveUploadTarget(session, parsed.data.locationType, parsed.data.locationId)
    const record = await analysisRecordService.recordQuality({
    ...
    logger.error({ err: error }, 'failed to store fruit quality assessment')
    return serverError()
  }
}
