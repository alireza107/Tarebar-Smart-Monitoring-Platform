...
import { fetchManagementLocations, fetchManagementOverview } from '@/modules/management-analytics/client'
...
export function LocationOverviewClient({ type, id }: { type: ManagementLocationType; id: string }) {
  ...
  return <div className="space-y-5">
    ...
    {overview.isLoading ? <div className="h-72 animate-pulse rounded-xl bg-muted" /> : overview.isError || !overview.data ? <EmptyAnalytics label="این مکان یافت نشد یا در محدوده دسترسی شما نیست." /> : <TabContent tab={tab} data={overview.data} />}
  </div>
}

function TabContent({ tab, data }: { tab: Tab; data: Awaited<ReturnType<typeof fetchManagementOverview>> }) {
  ...
  if (tab === 'spatial') return <Panel title="رفتار مکانی"><EmptyAnalytics label={data.spatialHeatmap.length ? 'نمای کامل نقشه در صفحه تحلیل مکانی در دسترس است.' : undefined} /></Panel>
